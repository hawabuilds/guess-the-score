...
function Eyebrow({ children }: { children: React.ReactNode }) {
  return (
    <p className="text-xs font-bold tracking-[0.25em] uppercase text-green-500 mb-3">
      {children}
    </p>
  );
}
...
function Card({
  ...
  return (
    <div
      className={`rounded-2xl border border-green-900/60 bg-gradient-to-b from-emerald-950/40 to-black/40 p-6 ${className}`}
    >
      {children}
    </div>
  );
...
function Callout({ children }: { children: React.ReactNode }) {
  return (
    <div className="rounded-2xl border-l-4 border-green-500 bg-green-500/10 px-6 py-4 text-green-100/90 leading-relaxed">
      {children}
    </div>
  );
}
...
export default function DocsContent() {
  ...
  const rich = {
    ...
    mono: (chunks: React.ReactNode) => (
      <span className="font-mono text-green-300 text-sm">{chunks}</span>
    ),
    xHandle: (chunks: React.ReactNode) => (
      <span className="text-green-300 font-semibold">{chunks}</span>
    ),
    site: (chunks: React.ReactNode) => (
      <span className="text-green-300 font-semibold">{chunks}</span>
    ),
    highlight: (chunks: React.ReactNode) => (
      <strong className="text-green-300">{chunks}</strong>
    ),
  };
  ...
  return (
    <div
      ...
      style={{
        ...
        background:
          "radial-gradient(ellipse 100% 60% at 70% -10%, rgba(60, 163, 63, 0.16), transparent 50%), #050508",
      }}
    >
      <header className="sticky top-0 z-50 flex justify-end px-4 pt-[calc(12px+env(safe-area-inset-top))] lg:hidden">
        <LanguageToggle variant="header" />
      </header>
      <header className="hidden lg:flex justify-end px-8 pt-8">
        <LanguageToggle variant="header" />
      </header>
      <nav className="lg:hidden sticky top-0 z-40 backdrop-blur-md bg-zinc-950/60 border-b border-green-900/50">
            ...
            <button
              ...
              className={`whitespace-nowrap rounded-full px-3.5 py-1.5 text-xs font-semibold border transition-colors ${
                active === s.id
                  ...
                  : "border-green-900/70 text-green-200/70 hover:border-green-600"
              }`}
            >
              {s.label}
            </button>
      ...
      <div className="mx-auto max-w-7xl lg:grid lg:grid-cols-[260px_1fr] lg:gap-12 px-4 sm:px-6 lg:px-8">
        <aside className="hidden lg:block">
          <div className="sticky top-8 py-10">
            ...
            <p className="text-xs text-green-200/50 mt-1 mb-8">
              {t("sidebarTagline")}
            </p>
                  ...
                  <button
                    ...
                    className={`group flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm text-left transition-colors ${
                      active === s.id
                        ...
                        : "text-neutral-400 hover:text-green-200 hover:bg-white/5"
                    }`}
                  >
                    <span
                      className={`h-1.5 w-1.5 rounded-full transition-colors ${
                        active === s.id
                          ? "bg-green-400"
                          : "bg-green-900 group-hover:bg-green-700"
                      }`}
                    />
                    {s.label}
                  </button>
            ...
            <div className="mt-10 border-t border-green-900/50 pt-5 text-xs text-green-200/40 leading-relaxed">
              guessthescore.xyz
              <br />
              @guessthescoreX · BNB Chain
            </div>
          </div>
        </aside>
        ...
        <main className="pb-24">
          <header className="pt-8 sm:pt-12 pb-16">
            <div className="flex flex-wrap items-center gap-2 mb-6">
              <span className="inline-flex items-center gap-1.5 rounded-full bg-green-500/15 border border-green-700/60 px-3 py-1 text-xs font-semibold text-green-300">
                <span className="h-1.5 w-1.5 rounded-full bg-green-400 animate-pulse" />
                {t("hero.live")}
              </span>
              <span className="rounded-full border border-green-900/70 px-3 py-1 text-xs font-semibold text-green-200/60">
                {t("hero.chain")}
              </span>
              <span className="rounded-full border border-green-900/70 px-3 py-1 text-xs font-semibold text-green-200/60">
                {t("hero.launchpad")}
              ...
              {(["step1", "step2", "step3", "step4", "step5", "step6"] as const).map(
                (stepKey, i) => (
                  <li key={stepKey} className="flex gap-4 items-start">
                    <span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-green-500/15 border border-green-700 text-green-300 text-xs font-bold">
                      {i + 1}
                    </span>
            ...
            <div className="grid sm:grid-cols-3 gap-4 max-w-3xl">
              {scoringCards.map((s) => (
                <Card key={s.key} className="text-center">
                  <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full border-2 border-green-500 bg-zinc-950/50 text-2xl font-extrabold text-green-400">
                    {s.pts}
                  </div>
            ...
            <div className="max-w-3xl space-y-3">
              {rewardTiers.map((tier, i) => (
                <Card key={tier} className="!p-5">
                  ...
                  <div className="mt-3 h-2 w-full rounded-full bg-green-950">
                    <div
                      className={`h-2 rounded-full bg-gradient-to-r from-emerald-600 to-blue-400 ${rewardWidths[i]}`}
                    />
                  </div>
                </Card>
              ))}
            ...
            <Card className="max-w-3xl !p-0 overflow-hidden">
              <dl className="divide-y divide-blue-900/50">
                {(["chain", "launchpad", "tax"] as const).map((row) => (
                  ...
                  >
                    <dt className="w-44 shrink-0 text-xs font-bold uppercase tracking-widest text-green-500">
                      {t(`token.table.${row}`)}
                    </dt>
          ...
          <section id="contract" className="scroll-mt-24 py-10">
            ...
            <div className="grid sm:grid-cols-3 gap-4 max-w-3xl">
              {claimSteps.map((key, i) => (
                <Card key={key}>
                  <div className="flex h-8 w-8 items-center justify-center rounded-full bg-green-500 text-black text-sm font-extrabold">
                    {i + 1}
                  </div>
                  <p className="mt-3 font-semibold text-white">
            ...
            <div className="grid sm:grid-cols-2 gap-3 max-w-3xl">
              {safetyRules.map((rule) => (
                <div
                  key={rule}
                  className="flex gap-3 items-start rounded-xl border border-green-900/50 bg-zinc-900/30 px-4 py-3"
                >
                  <svg
            ...
            <div className="grid sm:grid-cols-2 gap-4 max-w-3xl">
              <Card className="!p-5">
                ...
                <a
                  ...
                  className="mt-3 inline-block text-sm font-semibold text-green-400 hover:text-green-300"
                >
          ...
          <section id="links" className="scroll-mt-24 py-10">
            ...
            <div className="grid sm:grid-cols-2 gap-4 max-w-3xl">
              {officialLinks.map((link) => {
                const className =
                  "rounded-2xl border border-green-900/60 bg-zinc-900/30 p-5 hover:border-green-500 transition-colors";
                const content = (
                  <>
          ...
          <section id="disclaimer" className="scroll-mt-24 py-10">
            ...
            <footer className="mt-16 border-t border-green-900/50 pt-6 flex flex-wrap items-center gap-4">
              <div className="flex items-center gap-3">
                <DocsLogo className="h-8 w-auto" />
                <span className="text-sm text-neutral-500">
                  {t("brandName")}
                  <span className="text-green-500">.</span> · guessthescore.xyz
                </span>
              </div>
            </footer>
          </section>
