...
export function formatLandingPrizePool(
  stats: Pick<SiteStats, "prizePoolUsd" | "prizePoolUsdAtSnapshot">,
): string | null {
  return formatPrizePoolUsd(stats.prizePoolUsd, stats.prizePoolUsdAtSnapshot);
}

export function formatLandingPlayers(totalPlayers: number): string {
  return totalPlayers.toLocaleString();
}
