...
export async function getSiteStats(): Promise<SiteStats> {
  ...
  let prizePoolUsd = snapshotUsd;
  let prizePoolUsdAtSnapshot = snapshotUsd != null;

  if (
    prizePoolUsd == null &&
    Number.isFinite(prizePoolBnb) &&
    prizePoolBnb > 0
  ) {
    const bnbUsd = await withTimeout(fetchBnbUsdPrice(), ON_CHAIN_READ_TIMEOUT_MS);
    if (bnbUsd != null && bnbUsd > 0) {
      prizePoolUsd = prizePoolBnb * bnbUsd;
      prizePoolUsdAtSnapshot = false;
    }
  }
